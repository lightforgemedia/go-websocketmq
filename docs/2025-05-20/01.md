Okay, I've reviewed the code, especially focusing on how client-to-client proxying is implemented.

**Current Client Proxy Functionality Breakdown:**

1.  **Initiating Client (Source Client):**
    *   To send a request to another client (target client), the source client must:
        1.  Know the `TargetID` of the target client.
        2.  Know the specific `Topic` the target client is listening on for the desired action (e.g., "browser.navigate").
        3.  Marshal the actual request payload for that `Topic` into `json.RawMessage`.
        4.  Construct a `shared_types.ProxyRequest` struct containing `TargetID`, `Topic`, and the `json.RawMessage` payload.
        5.  Send this `ProxyRequest` as the payload of a standard server request to the broker, using the specific topic `shared_types.TopicProxyRequest` (`system:proxy`).
        6.  The response received from `SendServerRequest` will be a `*json.RawMessage`.
        7.  The source client must then manually unmarshal this `*json.RawMessage` into the expected response type from the *target client's handler*.

2.  **Broker:**
    *   The broker has a built-in handler for `shared_types.TopicProxyRequest`.
    *   When it receives a `ProxyRequest`:
        1.  It unmarshals the `ProxyRequest` struct.
        2.  It looks up the `TargetID` to get the target client's `ClientHandle`.
        3.  It then uses `targetClientHandle.SendClientRequest(...)` to forward the *inner* `Topic` and `json.RawMessage` payload (from `ProxyRequest.Payload`) to the target client.
        4.  It waits for the response from the target client. This response's payload will also be `json.RawMessage`.
        5.  It sends this `json.RawMessage` (received from the target client) back as the response to the source client's original `system:proxy` request.
    *   The broker essentially acts as a passthrough for the inner payload, ensuring it doesn't need to understand the application-specific types of the proxied messages. The `json.RawMessage` type is key here, as marshalling it just returns itself, preventing double-marshalling of the inner payload.

3.  **Target Client:**
    *   It has registered handlers for its application-specific topics (e.g., `browser.navigate`).
    *   When it receives the request forwarded by the broker:
        1.  It unmarshals the `json.RawMessage` payload into its specific request struct (e.g., `NavigateRequest`).
        2.  Its handler processes the request and prepares a response struct (e.g., `NavigateResponse`).
        3.  This response struct is marshalled and sent back to the broker.

**Critique of Current Design:**

*   **Verbosity for Source Client:** The source client has to do a lot of manual work: marshalling the inner payload, creating the `ProxyRequest` wrapper, and then unmarshalling the raw response.
*   **Lack of Type Safety for Source Client (Response):** The source client gets a `*json.RawMessage` back and must know the correct type to unmarshal it into. There's no compile-time guarantee this is correct.
*   **Exposure of Implementation Detail:** Clients needing to perform peer-to-peer requests must be aware of the `system:proxy` topic and the `ProxyRequest` structure.
*   **DRY Principle:** The pattern of wrapping/unwrapping for proxy requests might be repeated if many parts of the client code need to do this.

The current design is functional and the use of `json.RawMessage` for the proxied payload is efficient in avoiding re-marshalling by the broker. The main areas for improvement are on the client-side API to make it cleaner, safer, and more intuitive.

**Redesign Proposal for a Cleaner, More Direct, and DRYer Client Proxy:**

The core idea is to introduce a new, specialized method on the `client.Client` type that encapsulates the proxying logic, providing a more direct and type-safe API for the initiating client. The broker-side implementation can largely remain the same.

**1. New Client Method: `SendToClientRequest` (or `SendPeerRequest`)**

Add a new generic method to `pkg/client/client.go`:
...
**2. Broker-Side Handler (`shared_types.TopicProxyRequest` in `broker.go`)**

The existing broker handler for `system:proxy` is already well-suited for this and **does not need to change**:

```go
// In pkg/broker/broker.go -> setupDefaultHandlers()
err = b.HandleClientRequest(shared_types.TopicProxyRequest,
    func(src ClientHandle, req shared_types.ProxyRequest) (json.RawMessage, error) {
        ...
        if err != nil {
            return nil, err // Error: Target client not found
        }
        var resp json.RawMessage // This will hold the response from the target client
        // src.Context() is used for the overall operation lifetime.
        // req.Payload is already json.RawMessage from the source client.
        // req.Topic is the actual topic for the destination client.
        // The timeout here is for the broker-to-target-client request.
        if err := dest.SendClientRequest(src.Context(), req.Topic, req.Payload, &resp, b.config.serverRequestTimeout); err != nil {
            // This error could be because dest.SendClientRequest timed out,
            // or the target client 'dest' actively returned an error envelope for req.Topic.
            return nil, err
        }
        // 'resp' is the json.RawMessage payload from the target client's successful response.
        return resp, nil
    })
```
...
*   **Improved Client API:**
    *   Source clients now call `client.SendToClientRequest[ExpectedResponseType](targetID, actualTopic, actualPayload)`.
    *   This is more direct and clearly expresses the intent of client-to-client communication.
*   **Enhanced Type Safety:** The generic nature of `SendToClientRequest[T]` means the source client gets a compile-time typed response (`*T`), eliminating manual unmarshalling of `json.RawMessage` and the associated risks.
*   **Encapsulation:** The details of `system:proxy` topic and the `ProxyRequest` wrapper struct are hidden from the application developer using the client library. They become an internal implementation detail of `SendToClientRequest`.
*   **DRY:** The logic for preparing the `ProxyRequest` and handling the `json.RawMessage` response is centralized in the new client method.
*   **No Change to Broker Logic:** The broker's proxy handler remains efficient and doesn't need to understand application-specific types for the proxied messages.

**How to Use in Tests (Example from `TestClientProxyRequest`):**
...
// // ... manual unmarshal rawResp ...

// New way:
type helloReq struct { Msg string } // Request type for "demo.hello"
type helloResp struct { Reply string } // Response type for "demo.hello"

reqPayload := helloReq{Msg: "B"}
resp, err := client.SendToClientRequest[helloResp](clientB, context.Background(), clientA.ID(), "demo.hello", reqPayload)

if err != nil {
    t.Fatalf("SendToClientRequest failed: %v", err)
}
if resp.Reply != "hi B" { // Assuming clientA's "demo.hello" handler does this
    t.Errorf("unexpected reply '%s', expected 'hi B'", resp.Reply)
}
```

This redesign primarily enhances the client library's API for peer-to-peer requests, making it more robust and easier to use, without complicating the broker.
The output below will only include the modified `pkg/client/client.go` file and an updated `pkg/broker/broker_test.go` (`TestClientProxyRequest` part) to demonstrate the usage.
...
func TestClientProxyRequest(t *testing.T) {
	...
	clientA := testutil.NewTestClient(t, bs.WSURL, client.WithClientPingInterval(-1), client.WithClientName("ClientA"))
	clientB := testutil.NewTestClient(t, bs.WSURL, client.WithClientPingInterval(-1), client.WithClientName("ClientB"))

	// Client A handler for "demo.hello"
	err := clientA.HandleServerRequest("demo.hello", func(req helloReq) (helloResp, error) {
		t.Logf("ClientA: demo.hello handler invoked with: '%s'", req.Msg)
		return helloResp{Reply: "hi " + req.Msg}, nil
	})
	if err != nil {
		t.Fatalf("clientA handler registration for demo.hello failed: %v", err)
	}

	// Wait for both clients to connect and register
	...
	t.Logf("ClientA ID: %s, ClientB ID: %s", clientA.ID(), clientB.ID())

	// Client B (source) sends a request to Client A (target) using the new SendToClientRequest method
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	requestPayload := helloReq{Msg: "ClientB_To_ClientA"}
	resp, err := client.SendToClientRequest[helloResp](clientB, ctx, clientA.ID(), "demo.hello", requestPayload)

	if err != nil {
		t.Fatalf("client.SendToClientRequest failed: %v", err)
	}
	if resp == nil {
		t.Fatal("client.SendToClientRequest returned nil response")
	}

	expectedReply := "hi ClientB_To_ClientA"
	if resp.Reply != expectedReply {
		t.Errorf("unexpected reply: got '%v', want '%v'", resp.Reply, expectedReply)
	}
	t.Logf("ClientB successfully received typed response from ClientA: %+v", resp)
}
...
func TestListClients(t *testing.T) {
	...
	// Use the generic request since ListClients is a standard client-to-server request
	resp, err := client.GenericRequest[app_shared_types.ListClientsResponse](cli1, context.Background(), app_shared_types.TopicListClients, listReq)

	if err != nil {
		t.Fatalf("list clients request failed: %v", err)
	}
	if resp == nil {
		t.Fatal("list clients response is nil")
	}

	if len(resp.Clients) < 2 { // Can be more if other tests run concurrently or broker has other internal clients
		t.Errorf("expected at least 2 clients, got %d. Clients: %+v", len(resp.Clients), resp.Clients)
	}

	foundA := false
	foundB := false
	for _, c := range resp.Clients {
		if c.Name == "A" && c.ClientType == "browser" {
			foundA = true
		}
		if c.Name == "B" && c.ClientType == "service" {
			foundB = true
		}
	}
	if !foundA || !foundB {
		t.Errorf("Did not find both clients A and B in the list. FoundA: %t, FoundB: %t. List: %+v", foundA, foundB, resp.Clients)
	}
}
